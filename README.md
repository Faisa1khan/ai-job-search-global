...
- **`/pipeline`** runs the autonomous end-to-end job application workflow in a single turn: selects the highest-scoring unapplied match from the queue (or a provided URL), researches the company, tailors 1-page HTML CV and cover letter on Master templates, audits grounding against `CAREER_SOURCE_OF_TRUTH.md`, removes AI tells with the humanizer pass, compiles verified 1-page PDFs with headless Chromium, and records the application in `job_search_tracker.csv`. See [PIPELINE_GUIDE.md](PIPELINE_GUIDE.md) for the full architecture and reference.
