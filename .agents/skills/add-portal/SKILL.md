...
name: add-portal
description: >-
  Generate a job-portal search skill for a job board in your local market. Use when the user wants to add a new job portal or search source.
---

# /add-portal - Generate a Job-Portal Search Skill for Your Local Market

> Antigravity note: this skill is invoked as `/add-portal`. Any text, URL or argument the
...
- **`SKILL.md` frontmatter (AGY / Agent Skills standard):** `name` and a `description` written for skill triggering - it must name the portal, the market, and include trigger phrases in English **and** the market's language. Use AGY-appropriate tool references (`run_command` for the bun CLI, `search_web`/`read_url_content` for fallback). Do NOT emit `allowed-tools`, `context`, `version`, or `enabled` keys - AGY ignores them; tool access is governed by AGY's permission model, and the reactor's AppleScript/generator steps run under `run_command`.
...
1. Ask whether the user wants the new portal added to their `/scrape` search strategy. If yes:
   ...
   - Optionally add search_web/`site:` placeholder queries for that board in `.agents/skills/job-scraper/search-queries.md` (use the `[YOUR_JOB_BOARD]` style placeholders already there) so the fallback path still covers the board if the CLI is unavailable.
