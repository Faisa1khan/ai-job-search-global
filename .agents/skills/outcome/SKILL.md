...
name: outcome
description: >-
  Record the result of an application (progress updates, offers, rejections) into the tracker and the per-application archive.
---

# /outcome - Record the Result of an Application

> Antigravity note: this skill is invoked as `/outcome`. Any text, URL or argument the
...
2. **`job_posting.md`** - if it already exists, leave it. Otherwise try read_url_content on the tracker row's `source` URL and save the posting text, retrying a 403 with browser headers per `.agents/skills/job-application-assistant/09-web-research.md`. If the URL is dead (postings expire fast - this is exactly why the archive matters), ask the user to paste the posting, or write a stub noting the posting is unavailable. **Never reconstruct a posting from memory.**
