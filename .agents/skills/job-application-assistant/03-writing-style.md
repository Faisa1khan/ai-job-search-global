...
0. **DEFAULT PROSE PASS — HUMANIZER.** Run the humanizer skill (`.agents/skills/humanizer/SKILL.md`, `/humanizer`) over the final prose of every CV and cover letter. It is the default, non-optional check applied after drafting and before compiling. Its pattern list (below and in the skill) catches what these rules enumerate in shorthand: not-X-but-Y contrasts, one-line closers, dramatic fragments, staged run-ups, arguing with no one, forced triads, repeated sentence openings, em-dash overuse, stacked qualifiers, hyphenated pairs, passive/missing subjects, overused AI words, inflated significance, vague associations, shallow -ing riders, sales language, borrowed authority, copula avoidance, decorative bolding/headings, curly quotes, chatbot residue, knowledge-limit disclaimers, repeated headings, and writing-about-the-previous-version. The repo-specific exceptions from Step 4b of `/apply` apply (keep the locked design's en-dash bullets and `<strong>` keywords; content-prose edits only).
1. **NO em-dashes (--).**  Use commas, periods, or restructure the sentence instead. Exception: the frozen Master design's separator dashes (entry-title `.role` spans, `.edu-name` spans, award `<strong>` lead-ins) are design tokens, never prose, and are exempt from the linter.
