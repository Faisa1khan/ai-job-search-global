...
The resume visual design is permanently locked to the canonical HTML implementation:
- **Canonical Template (design + structure):** `cv/Resume_Master.html`
- **Reference Output:** `cv/Resume_Master.html` $\to$ `cv/Resume_Master.pdf` (1 page, letter format).
- **Grounding Single Source of Truth (FACTS):** `CAREER_SOURCE_OF_TRUTH.md` wins for every number/date/claim.
...
8. **Visual Regression Check:** Compare against the approved reference `cv/Resume_Master.html` $\to$ `cv/Resume_Master.pdf`.
