...
- Fetch the job posting content (use read_url_content for URLs). **A 403 is not a dead end** - follow the escalation order in `09-web-research.md` before concluding a page is unavailable, and prefer the employer's own careers posting over an aggregator listing
...
| `09-web-research.md` | Fetching postings and company pages: trust boundary, the read_url_content 403 fallback, escalation order, claim verification |
