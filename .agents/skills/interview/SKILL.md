---
name: interview
description: "Prepare for a scheduled interview on a tracked application: stage-specific prep pack plus optional mock interview."
---

# /interview - Prepare for an Interview on a Tracked Application

> Antigravity note: this skill is invoked as `/interview`. Any text, URL or argument the
...
**Verify before using:** every company claim that will appear in the prep pack must be independently confirmed via read_url_content/search_web - same rule the repo applies to cover-letter claims. An unverified "fact" delivered confidently in an interview is worse than no fact. On a 403, retry with browser headers per `.agents/skills/job-application-assistant/09-web-research.md` rather than dropping to search snippets; a snippet is a lead, not a source.
