...
name: rank
description: >-
  Triage scraped jobs into a ranked shortlist by scoring each posting against the fit framework.
---

# /rank - Triage Scraped Jobs into a Ranked Shortlist

> Antigravity note: this skill is invoked as `/rank`. Any text, URL or argument the
...
Dispatch parallel `general-purpose` agents via the **invoke_subagent**, ~5 jobs per agent (a single agent is fine for ≤5 jobs). Token-efficiency rules, consistent with `/apply`:
