...
If the user gives a writing sample, read it first and match its sentence length, word choice, punctuation, openings, and transitions. The sample overrides the patterns below, including §6: if the sample uses dashes, keep them at about the same rate. For a resume and cover letter the writing sample is the candidate's own profile prose and previous tailored documents; keep the professional register of the locked design while removing AI tells.

Without a sample, take the voice from the kind of text. Reference, technical, and factual text (resume bullets, cover letters, specs) stays neutral and plain. Removing tells is half the job; the result must still sound like a person.

### What to return

**Pasted text (default).** Return the draft, a short list of remaining patterns, and the final rewrite.

**File mode.** When the user names a file, run the full process but write only the final text to the file. Change prose only. Keep code blocks, inline code, commands, paths, HTML/CSS markup, YAML metadata, data, and link targets unchanged. Then give the user a short summary.

**Embedded mode.** When another task uses this skill (resume, cover letter, PR, commit message, document), return only the final text.
...
**Rule:** The final rewrite must not contain em dashes (—) or en dashes (–) unless the source text's own register already uses them for a reason; this repo's locked resume/cover-letter design forbids them in prose, so for that output a resume or cover letter must contain no em or en dashes. Replace each dash with a period, comma, colon, or parentheses, or rewrite the sentence. This includes spaced dashes and double hyphens (` -- `) used as dashes. Leave dashes and hyphens inside code blocks, inline code, commands, paths, and URLs alone.
...
**Problem:** These pairs are hyphenated in every position. Keep the hyphen before a noun when grammar needs it, as in `a high-quality report`, and drop it after the noun, as in `the report is high quality`. *Weak alone.* (Note: the locked resume design intentionally uses en-dash bullet markers `–` for list bullets — those are layout, not prose, and stay.)
**Before:**
...
**Problem:** Words are bolded without a reason, and vertical lists give every item a bold label and a colon. Remove the bold. Turn a labeled list into prose when the labels carry no information of their own. (Note: the locked resume design preserves intentional `<strong>` keyword highlighting — that is required and stays; this pattern applies only to decorative, redundant bolding.)
**Before:**
...
**Problem:** Headings capitalize every main word, and headings or list items carry emojis or arrows (→) as decoration. A horizontal rule sits between every section, or the document opens with a top-level heading that repeats its own title. Use sentence case, remove the decoration and the rules, and let the title stand once. (Note: the locked resume/cover-letter design fixes its own heading style; do not alter it — invest any AI-tell fixes in main content prose.)
**Before:**
...
## F. Style, rhythm, and rhetoric (additional)

Four tells that upstream does not cover but real AI prose still shows. Unlike §1 to §5, none of these justify an edit on a single sighting; treat them as *weak alone* unless they pile up in the same passage.

### F1. Forced metaphors and figurative overwriting
...
**Problem:** Beyond the stock figurative words flagged in §12 and §16, some text invents decorative metaphors that add imagery without adding meaning, then often explains them. Plain description is usually clearer and more honest. If the metaphor does not earn its place, cut it and say the literal thing.
**Before:**
...
### F2. Rhetorical questions answered immediately

**Signs to watch:** "What if...?", "The question is...", "Ever wondered...?", a question immediately followed by its own answer, "Think about it."
**Problem:** Text poses a question only to answer it a beat later. The question adds no information and stalls the sentence. State the point directly.
**Before:**
...
### F3. Sentence-opener tics

**Signs to watch:** So..., Look,, habitual sentence-initial And/But, "I think"/"I believe" when stating a fact, adverb openers (Interestingly, Importantly, Notably, Crucially, Essentially, Ultimately).
**Problem:** A small set of openers gets leaned on. Adverb openers tell the reader how to feel instead of earning it, and "So" or "Look" fake conversational warmth. Drop the opener and start with the substance.
**Before:**
...
**Problem:** Reassurance gets tacked on that the reader never asked for. It softens the writing and assumes the reader needs comforting. Trust the reader: make the point and stop.
**Before:**
...
### F5. Marketing and blog cliches

**Signs to watch:** at the end of the day, when it comes to, in a world where, moving forward, circle back, deep dive, game-changer, double down, take a step back, on the same page, make no mistake, it turns out, let me be clear, navigate (for challenges), lean into, unpack (before analysis), straightforward (to describe anything).
**Problem:** Same tell as §12 in a different register. When one of these shows up, write the plain claim underneath it.
...
## Adding personality when the tells are gone

Stripping the patterns above is half the job. Sterile, voiceless prose is just as detectable as slop, and it reads badly even when no one is scoring it. The Voice section already tells you to match the writer; when you have no sample, these moves give the result a pulse:

- **Have opinions.** Report the facts, then react to them. "I genuinely don't know how to feel about this" reads more human than a neutral pros/cons list.
- **Vary rhythm.** Short punchy sentence. Then a longer one that takes its time. Mixing them is the most reliable sign a person wrote it.
- **Acknowledge complexity.** "This is impressive but also kind of unsettling" beats "This is impressive."
- **Use "I" when it fits.** First person reads as honest in most prose. "I keep coming back to..." or "Here's what gets me..." signals a real person thinking. (For resumes and cover letters, first-person is limited to the cover letter; resume bullets stay ego-dropped and verb-led per the locked style.)
...
1. Read the input text carefully (read the file if it is one).
2. Mark every tell you find, strongest first, including paragraph shape.
3. Draft the rewrite. Keep every supported claim; do not add facts, names, numbers, dates, quotes, or citations the source or the user did not give.
4. Check the draft aloud. Note anything that still sounds AI-generated and any claim the rewrite added or dropped.
5. Search for the five survivors: a not-X-but-Y contrast, a one-line closer, a dash, a triad, a bold label.
6. Write the final version, stating each point naturally and varying sentence length.
7. For pasted text, return the draft, the short list of remaining tells, and the final rewrite. For a file, write only the final text (prose only; leave code, data, markup, and link targets alone) and give a short summary. For embedded use (resume, cover letter, PR, commit message, doc), return only the final text.
...
This skill is ported from [blader/humanizer](https://github.com/blader/humanizer) (MIT licensed), which is itself based on Wikipedia's "Signs of AI writing", maintained by WikiProject AI Cleanup. Original author: Siqi Chen (@blader). Original repo: https://github.com/blader/humanizer. Upstream v3.0.0 body reproduced with repo-specific notes (locked resume/cover-letter design exceptions) added in the Voice, §8, §10, §19, §20, and "Adding personality" sections.