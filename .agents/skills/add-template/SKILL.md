...
name: add-template
description: >-
  Register a custom CV or cover-letter template (LaTeX, Typst, or any CLI-compiled toolchain). Use when the user wants to swap in their own application template.
---

# /add-template - Register a Custom CV or Cover Letter Template

> Antigravity note: this skill is invoked as `/add-template`. Any text, URL or argument the
