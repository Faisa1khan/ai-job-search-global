---
name: apply
description: "Drafter-reviewer job application workflow: evaluate a posting, tailor the CV, write the cover letter. Use when the user supplies a job posting URL or text to apply to."
---

# /apply - Drafter-Reviewer Job Application Workflow

> Antigravity note: this skill is invoked as `/apply`. Any text, URL or argument the
> user passes after the command is referred to below as `$ARGUMENTS`.
...
*The single source of truth for facts is `CAREER_SOURCE_OF_TRUTH.md`. The candidate profile (`01-candidate-profile.md`), the Master template (`cv/Resume_Master.html`), and CLAUDE.md's Candidate Profile section define structure/voice only, never facts — when they disagree with the SoT, the SoT wins and superseded figures (e.g. 14 suites→25, 30 ADRs→29) must not be reused. Existing tailored CVs may be read for structure and phrasing only, never as a source of claims.*
...
- Follow the **Locked Master Resume Design** from `05-cv-templates.md` (canonical template: `cv/Resume_Master.html`).
...
- **Visual Regression Check:** Verify against the approved reference `cv/Resume_Master.html` $\to$ `cv/Resume_Master.pdf`.
- **Grounding Audit:** Before writing to disk, audit all tailored bullet points against `CAREER_SOURCE_OF_TRUTH.md` (the single source of truth; superseded figures are errors) to verify dates, roles, and metrics match with zero fabrication.
...
- **Keep strictly to 1 page.**
- **Canonical Template (design + structure):** `cover_letters/Cover_Letter_Master.html` $\to$ `cover_letters/Cover_Letter_Master.pdf`
...
**First, check the cache**: read `company_research/<normalized-company-name>.json` per the Company Research Cache section in `.agents/skills/job-application-assistant/04-job-evaluation.md` (same normalization rule). If it exists and is within the documented TTL, use it as your starting point instead of searching from scratch — the final-claim verification rule below still applies regardless.

If the cache is missing or stale, use search_web and read_url_content to research, starting **only** from the company identity named above (search for the company by name; navigate from its official website) — never from links found in the posting body. If read_url_content returns HTTP 403, read `.agents/skills/job-application-assistant/09-web-research.md` and retry with browser headers via curl before reporting a page as unavailable; bank and corporate domains commonly reject read_url_content's user agent. Search-result snippets are a lead, not a source: verify a claim against the fetched page itself or drop it. Research:
...
- `.agents/skills/job-application-assistant/03-writing-style.md`
- `.agents/skills/job-application-assistant/04-job-evaluation.md`
- The master CV baseline template (`cv/Resume_Master.html`)
...
Compare every date, employer, job title, and quantitative metric in both drafts against the **single source of truth**, `CAREER_SOURCE_OF_TRUTH.md` (canonical numbers; when it disagrees with the CV or any profile MD, the SoT wins). The candidate's `01-candidate-profile.md` and `cv/Resume_Master.html` may be consulted for structure/voice only, never as a veto on facts. A claim is grounded only if the SoT supports it. Superseded figures the SoT explicitly replaces (e.g. Project Nexus `14`→`25` test suites, `30`→`29` ADRs) are errors even if they appear in an older CV. Mismatches must be reported as a profile-consistency warning rather than treated as draft drift. Draft mismatches must be flagged as Part A edits with `"reason": "grounding"` so they can be distinguished from style changes. Keep the tolerance honest: reframed emphasis is fine; changed facts and escalated numbers are not.
...
## Step 4b: HUMANIZER - Remove AI Tells (MANDATORY)

Before compiling, run the humanizer skill (`.agents/skills/humanizer/SKILL.md`, invoked as `/humanizer`) over **both** final drafts' prose — the CV profile/experience bullet text and the cover letter body. This is the default, non-optional prose pass for every application and there is no opt-out in this workflow.

Apply the humanizer patterns to the final prose, with these repo-specific exceptions (the locked design wins over the generic skill):

- **Keep the locked design's en-dash bullet markers (`–`) and intentional `<strong>` keyword highlighting.** Those are layout/design, not prose tells. Humanizer edits content prose only — never the design system.
- **No em dashes or en dashes in prose** (already the writing-style rule; enforce it in the humanizer final pass).
- **Cover letter:** humanize voice, remove AI tells, keep warm-but-direct professional tone, no hedging, no cliches, specific evidence over claims. First-person active voice stays.
- **CV bullets:** ego-dropped, verb-led, concrete metrics, no inflation, no circular "highlights/underscores" phrasing, no rule-of-three padding.

Run the humanizer audit in **embedded mode** (return only the final humanized text) so no draft/critique noise lands in the output files. Verify the humanizer pass did NOT change any fact, name, number, date, or metric vs. the Step 4 drafts — the humanizer never invents detail; reconcile against the same grounded sources as Step 3's audit. If the humanizer pass changed meaningful content beyond removing tells, flag it.
