...
`.agents/skills/` (plus search_web as a fallback), using queries from your profile.
...
**Use the installed CLI tools as the primary search mechanism.** Fall back to `search_web` only for portals that do not have a CLI skill, or if `bun` is unavailable on the system.
...
If this fails (bun not installed), skip to **1c (search_web fallback)** for all portals and note the fallback in the Step 5 output.
...
Run all portal CLI calls in parallel where possible using the invoke_subagent. Collect all `results` arrays into a single pool for Step 2, keeping each result tagged with its source portal skill (for Step 2 `detail` lookups).
...
Use the site-specific query strings from `search-queries.md` directly as search_web queries for these portals.

Tag each fallback result as search_web-sourced, keeping the portal tag when the fallback stands in for an installed portal whose CLI failed. Step 4 persists this as the entry's `source`, and Step 5 reports which portals ran on the fallback this run.
...
**From search_web results:** Use `read_url_content` on the posting URL and extract the same
fields manually. If it returns HTTP 403, retry with browser headers via curl per
`.agents/skills/job-application-assistant/09-web-research.md` before giving up — most
bank and corporate sites reject read_url_content's user agent while serving browsers normally.
...
that entry. When search_web only yields a listing page, search the employer's own careers
...
1. **Never fabricate job postings.** Only present jobs from actual CLI search/detail output or search_web/read_url_content results.
2. **Respect deduplication.** Always check seen_jobs.json AND job_search_tracker.csv before presenting.
3. **Focus on configured geographic area.** Skip jobs that require relocation or are clearly outside commute range.
4. **Only open positions.** Skip postings with expired deadlines or those marked as closed.
5. **Be efficient with detail fetches.** Don't run `detail` or read_url_content on every search hit — pre-filter by title/snippet, then fetch only promising matches.
6. **Parallel searches.** Run portal CLI searches in parallel; use search_web only for gaps the CLIs don't cover.
